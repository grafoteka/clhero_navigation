...
#define POS_ANGULAR_GAIT_PATTERN_NAME "turn_left_tripod"
#define NEG_ANGULAR_GAIT_PATTERN_NAME "turn_right_tripod"
...
void nav_command_msg_callback (const geometry_msgs::Twist::ConstPtr& msg){
	...
	if(fabs(vx) > fabs(w)){
		...
		if(fabs(w) > MIN_ANGULAR_VELOCITY){
			...
			//If something went wrong exit the callback
			if(checkErrorsGaitCommand(command_msg.response.ans)){
				ROS_INFO("[clhero_vel_interface] Update parameters failed on %s pattern", command_msg.request.pattern_name);
				return;
			}
			//Now checks which gait pattern is currently active
			if(current_gait_pattern.compare(command_msg.request.pattern_name) != 0){
				...
				clhero_command_client.call(start_pattern_msg);
				//If something went wrong exit the callback
				if(checkErrorsGaitCommand(start_pattern_msg.response.ans)){
					ROS_INFO("[clhero_vel_interface] Unable to start pattern: %s", start_pattern_msg.request.pattern_name.c_str());
					return;
				}else{
					ROS_INFO("[clhero_vel_interface] Pattern %s successfully started.", start_pattern_msg.request.pattern_name.c_str());
